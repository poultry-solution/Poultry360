...
export const createVerificationRequest = async (
  ...
  try {
    ...
    // Check rejection restrictions
    const rejectedRequests = await prismaWithVerification.dealerVerificationRequest.findMany({
      where: {
        dealerId: dealer.id,
        companyId: companyId,
    ...
    if (rejectedRequests.length > 0) {
      ...
      if (lastRejection.lastRejectedAt) {
        ...
        if (lastRejection.lastRejectedAt > oneHourAgo) {
          const minutesRemaining = Math.ceil(
            (lastRejection.lastRejectedAt.getTime() - oneHourAgo.getTime()) /
              (60 * 1000)
          );
          return res.status(400).json({
            success: false,
