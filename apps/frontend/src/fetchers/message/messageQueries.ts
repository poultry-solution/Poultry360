...
export const useEditMessage = (conversationId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (res) => {
      // Optimistically update conversation cache if present
      queryClient.setQueriesData({ queryKey: messageKeys.conversation(conversationId) }, (old: any) => {
        if (!old) return old;
        const updated = res.message;
        const replace = (arr: any[]) => arr.map((m: any) => (m.id === updated.id ? updated : m));
        if (Array.isArray(old.messages)) {
          return { ...old, messages: replace(old.messages) };
        }
        return old;
      });
      queryClient.invalidateQueries({
...
export const useDeleteMessage = (conversationId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (_res, messageId) => {
      ...
      queryClient.setQueriesData({ queryKey: messageKeys.conversation(conversationId) }, (old: any) => {
        if (!old) return old;
        const filterOut = (arr: any[]) => arr.filter((m: any) => m.id !== messageId);
        if (Array.isArray(old.messages)) {
          return { ...old, messages: filterOut(old.messages) };
        }
        return old;
      });
      queryClient.invalidateQueries({ queryKey: messageKeys.conversationPage(conversationId, 1, 50) });
