...
export default function CompanyLayout({
  ...
  // Redirect to company dashboard if on root company path
  useEffect(() => {
    if (isAuthenticated && user && pathname === "/company") {
  ...
  return (
    <AuthGuard
      requireAuth={true}
      allowedRoles={["OWNER", "MANAGER", "COMPANY"]} // Phase 2A: Using OWNER role for companies until COMPANY role is properly configured
    >
      {children}
    </AuthGuard>
  );
