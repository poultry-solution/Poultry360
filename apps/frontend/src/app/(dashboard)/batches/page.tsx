...
export default function BatchesPage() {
  ...
  const [batches, setBatches] = useState<BatchItem[]>([
    { id: 1, code: "B-2024-001", farm: "Farm A", startDate: "2024-01-15", initialBirds: 2500, status: "Active" },
    { id: 2, code: "B-2024-002", farm: "Farm B", startDate: "2024-01-20", initialBirds: 2000, status: "Active" },
  ]);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const newBatch: BatchItem = {
      id: batches.length + 1,
      code: formData.code || `B-${new Date().getFullYear()}-${String(batches.length + 1).padStart(3, "0")}`,
    ...
    setFormData({ code: "", farm: farms[0] ?? "", startDate: "", initialBirds: "", notes: "" });
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button className="bg-primary hover:bg-primary/90 cursor-pointer" onClick={() => setIsModalOpen(true)}>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium group-hover:text-primary-foreground">Active Batches</CardTitle>
            <Layers className="h-4 w-4 text-muted-foreground group-hover:text-primary-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold group-hover:text-primary-foreground">{activeBatches.length}</div>
            <p className="text-xs text-muted-foreground group-hover:text-primary-foreground">Currently running</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{batches.reduce((sum, b) => sum + b.initialBirds, 0)}</div>
        ...
        >
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium group-hover:text-primary-foreground">Closed Batches</CardTitle>
            <Layers className="h-4 w-4 text-muted-foreground group-hover:text-primary-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold group-hover:text-primary-foreground">{closedBatches.length}</div>
            <p className="text-xs text-muted-foreground group-hover:text-primary-foreground">Till now</p>
          </CardContent>
        </Card>
      </div>
      ...
      {/* New Batch Modal */}
      <Modal isOpen={isModalOpen} onClose={handleClose} title="Create New Batch">
        <form onSubmit={handleSubmit}>
          <ModalContent>
            <div className="space-y-4">
              <div>
                <Label htmlFor="code">Batch Code</Label>
                <Input id="code" name="code" value={formData.code} onChange={handleChange} placeholder="e.g., B-2024-003" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input id="initialBirds" name="initialBirds" type="number" value={formData.initialBirds} onChange={handleChange} placeholder="e.g., 2500" required />
              </div>
              <div>
                <Label htmlFor="notes">Notes (optional)</Label>
                <textarea id="notes" name="notes" value={formData.notes} onChange={handleChange} className="w-full px-3 py-2 border border-input bg-background rounded-md text-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" rows={3} placeholder="Any remarks" />
              </div>
            </div>
          </ModalContent>
          <ModalFooter>
            <Button type="button" variant="outline" onClick={handleClose}>Cancel</Button>
            <Button type="submit" className="bg-primary hover:bg-primary/90">Create Batch</Button>
          </ModalFooter>
        </form>
      </Modal>
      ...
      >
        ...
        <ModalFooter>
          <Button variant="outline" onClick={() => setIsCountModalOpen(false)}>Close</Button>
        </ModalFooter>
      </Modal>
    </div>
  );
