...
function actorRoleLabel(log: BusinessAuditLog) {
  return log.actorType === "STAFF" ? "Staff" : log.actorRole === "DEALER" ? "Owner" : log.actorRole || "User";
}

function actorDisplayName(log: BusinessAuditLog, scope: "dealer" | "admin", dealerBusinessName?: string) {
  if (scope === "dealer" && log.actorType === "USER" && log.actorRole === "DEALER" && dealerBusinessName) {
    return `${dealerBusinessName} — Owner (${log.actorName})`;
  }
  return log.actorName;
}

function rows(logs: BusinessAuditLog[], scope: "dealer" | "admin", dealerBusinessName?: string) {
  return logs.map((log) => [new Date(log.createdAt).toLocaleString(), actorDisplayName(log, scope, dealerBusinessName), actorRoleLabel(log), log.description, recordLabels[log.targetType] || "Business record", readableDetails(log.metadata, log.securityMetadata)]);
}

function exportCsv(logs: BusinessAuditLog[], scope: "dealer" | "admin", dealerBusinessName?: string) {
  download([[["Time", "Actor", "Role", "Action", "Related to", "Details"], ...rows(logs, scope, dealerBusinessName)].map((row) => row.map(escapeCsv).join(",")).join("\n")], "text/csv;charset=utf-8", "activity.csv");
}
function exportExcel(logs: BusinessAuditLog[], scope: "dealer" | "admin", dealerBusinessName?: string) {
  const cells = [["Time", "Actor", "Role", "Action", "Related to", "Details"], ...rows(logs, scope, dealerBusinessName)].map((row) => `<tr>${row.map((value) => `<td>${String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;")}</td>`).join("")}</tr>`).join("");
  download([`<html><head><meta charset="utf-8"></head><body><table>${cells}</table></body></html>`], "application/vnd.ms-excel", "activity.xls");
}
function exportPdf(logs: BusinessAuditLog[], scope: "dealer" | "admin", dealerBusinessName?: string) {
  const lines = rows(logs, scope, dealerBusinessName).map((row) => `${row[0]} | ${row[1]} | ${row[3]} | ${row[4]} | ${row[5]}`.replace(/[()\\]/g, "\\$&").slice(0, 140));
...
export function BusinessActivityPage({ scope }: { scope: "dealer" | "admin" }) {
  ...
  const dealerBusinessName =
    scope === "dealer"
      ? user?.dealer?.name || user?.companyName || undefined
      : undefined;
  ...
  const doExport = async (type: "csv" | "excel" | "pdf") => {
    try {
      ...
      if (type === "csv") exportCsv(all, scope, dealerBusinessName); else if (type === "excel") exportExcel(all, scope, dealerBusinessName); else exportPdf(all, scope, dealerBusinessName);
